...
adm1_colnames <- colnames(final_children_adm1)

adm1_colnames <- adm1_colnames[5:11]

final_children_adm1_wide <- tidyr::pivot_wider(final_children_adm1, id_cols = NAME_1, names_from = c(age_group, antigen), names_sep = "_", values_from = all_of(adm1_colnames))

final_children_adm1_wide <- left_join(adm1_code, final_children_adm1_wide, by = "NAME_1")

adm2_colnames <- colnames(final_children_adm2)

adm2_colnames <- adm2_colnames[5:11]

final_children_adm2_wide <- tidyr::pivot_wider(final_children_adm2, id_cols = NAME_2, names_from = c(age_group, antigen), names_sep = "_", values_from = all_of(adm2_colnames), values_fn = length)
...
write.csv(final_children_adm1_wide, "Output/Children_PopRisk_LGA.csv")
